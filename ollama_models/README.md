...
- **probe**: Probe and record the maximum context size that fits in VRAM for each model.
  - Both commands support `--ignore` to load a plain text ignore list (default: `./ollama_models_ignore.conf`).
  - Any model listed in the ignore file is skipped, including when `--model` is used.
  - If the ignore file does not exist, commands continue normally as if there are no ignored models.
...
ollama-models context usage --ignore ./ollama_models_ignore.conf
ollama-models context probe --ignore ./ollama_models_ignore.conf
```
...
- `ollama_models_ignore.conf`: Optional plain text ignore list for `context usage` and `context probe` (one model per line). Blank lines and `#` comments are ignored. If the file is absent, no models are ignored.
